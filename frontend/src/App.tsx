...
export default function App() {
  ...
  return <main className="shell">
    <header><p className="eyebrow">PUBLIC DOCUMENT UTILITY</p><h1>Omgevingsloket Document Downloader</h1><p>Find and download only files that the public Inzageloket makes available.</p></header>
    ...
    {project && !discovering && <section className="documents">
      ...
      {documents.length === 0 ? <p className="notice">No publicly accessible documents were found.</p> : <>
        ...
        <footer className="selection"><span>{selected.size} document{selected.size === 1 ? '' : 's'} selected</span><div><button className="secondary" onClick={() => setSelected(new Set())}>Clear selection</button><button className="secondary" onClick={() => download(documents.filter((document) => document.downloadable).map((document) => document.id))}>Download all</button><button onClick={() => download()} disabled={!selected.size}>Download selected</button></div></footer>
      </>}
    </section>}
    {job && <section className="job"><h2>{job.status === 'running' ? `Downloading ${job.completed} / ${job.total}` : job.status === 'completed' ? 'Archive ready' : 'Archive failed'}</h2><p>Downloaded: {job.completed - job.failures.length} · Failed: {job.failures.length}</p>{job.downloadUrl && <a className="button-link" href={job.downloadUrl}>Download ZIP</a>}{job.failures.length > 0 && <button onClick={() => download(job.failures.map((failure) => failure.documentId))}>Retry failed documents</button>}</section>}
  </main>;
}
