...
export interface ObjectStore { put(key: string, stream: Readable, contentType?: string): Promise<PutResult>; get(key: string): Promise<Readable>; exists(key: string): Promise<boolean>; signedDownloadUrl(key: string, filename: string): Promise<string>; delete(key: string): Promise<void> }
...
export class R2ObjectStore implements ObjectStore {
  ...
  async get(key: string): Promise<Readable> {
    ...
    if (!response.Body) throw new Error('Archive object has no readable body.');
...
export class MemoryObjectStore implements ObjectStore {
  ...
  async get(key: string): Promise<Readable> { const value = this.values.get(key); if (!value) throw new Error('Archive object not found.'); return Readable.from(value); }
