export interface ArchiveProject { id: string; projectNumber: string; title?: string; municipality?: string; status?: string; publicationType?: string; source?: 'archive' | 'live'; isCurrentlyPublic: boolean; firstSeenAt: string; lastSeenAt: string; lastCrawledAt?: string; documentCount?: number }
export interface ArchiveDocument { id: string; projectNumber: string; upstreamUuid: string; filename: string; description?: string; category?: string; mimeType?: string; sizeBytes?: number; source?: 'archive' | 'live'; downloadable: boolean; downloadStatus: string; storageKey?: string; firstSeenAt: string; viewerUrl?: string }
